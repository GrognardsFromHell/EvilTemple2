...
// By defining OPTIMIZE_LITTLE_ENDIAN, the program will try to
// read structures from the stream en block if the byte order
// of the current system is LITTLE_ENDIAN (should almost always
// be the case).
//#define OPTIMIZE_LITTLE_ENDIAN 1
...
namespace EvilTemple
{
    ...
    public:
        ...
        QSharedPointer<MeshModel> read() {
            ...
            if (skmData.isNull() || skaData.isNull()) {
                ...
                MeshModel *result = new MeshModel(QList< QSharedPointer<FaceGroup> >(), QVector<Vertex>());
                return QSharedPointer<MeshModel>(result);
            }
            ...
            QSharedPointer<MeshModel> result(new MeshModel(faceGroups, vertices, skeleton.take()));
...
#if OPTIMIZE_LITTLE_ENDIAN && Q_BYTE_ORDER == Q_LITTLE_ENDIAN
            stream.readRawData((char*)&header, sizeof(quint32) * 8);
#else
...
#if OPTIMIZE_LITTLE_ENDIAN && Q_BYTE_ORDER == Q_LITTLE_ENDIAN
                stream.readRawData((char*)&vertex, VertexSize);
#else
...
#if OPTIMIZE_LITTLE_ENDIAN && Q_BYTE_ORDER == Q_LITTLE_ENDIAN
                const quint32 faceWithMaterialSize = sizeof(quint16) + sizeof(Face); // Avoid pragma pack
                stream.readRawData((char*)&faceWithMaterial, faceWithMaterialSize);
#else
    ...
    QSharedPointer<MeshModel> SkmReader::get()
    {
        return d_ptr->read();
    }
